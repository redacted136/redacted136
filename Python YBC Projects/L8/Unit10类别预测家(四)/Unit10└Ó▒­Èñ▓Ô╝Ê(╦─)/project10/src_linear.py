...
data_ori = pd.read_csv('linear_svm.csv')
train_data = numpy.array(data_ori[[u'生命值', u'法力值', u'攻击力', u'防御力', u'敏捷']])

train_target = numpy.array(data_ori[[u'类型']][u'类型'].map({'魔法':0,'物理':1}))   #魔法类型对应0、物理类型对应1。
...
def main(ml):
    ...
    text_font1 = pygame.font.Font('images/font/a.TTF', 25)
    ...
    button_icon_rect.center = (width / 2, height / 2 + 250)
    ...
    image_w = pygame.image.load('images/物理系.png')
    image_w_rect = image_w.get_rect()
    image_w_rect.center = (width / 2, height / 2)

    image_m = pygame.image.load('images/魔法系.png')
