...
data_ori = pd.read_csv('./height_data3.csv')

train_data = numpy.array(data_ori[[u'父亲身高', u'母亲身高', u'孩子性别']])
    ...
    while True:
        for event in pygame.event.get():
            ...
            if Gamestate == 2:  # 母亲身高
                ...
                screen.fill(bg)
                screen.blit(image_background2, (0, 0))
                txtSurface = font.render(text, True, colour)
                width = max(200, txtSurface.get_width() + 10)
                inputBox.w = width
                screen.blit(txtSurface, (inputBox.x + 5, inputBox.y + 5))
            ...
            if Gamestate == 3:  # 性别
                ...
                screen.fill(bg)
                screen.blit(image_background3, (0, 0))
                txtSurface = font.render(text, True, colour)
                width = max(200, txtSurface.get_width() + 10)
                inputBox.w = width
                screen.blit(txtSurface, (inputBox.x + 5, inputBox.y + 5))
